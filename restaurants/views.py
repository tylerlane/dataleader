...
def index( request ):
  ...
  inspections = Inspection.objects.select_related().filter(date__gt=datetime.datetime.today() - datetime.timedelta(days=int(days_since)))

  violations = inspections.order_by('-critical')[:10]

  return render_to_response( 'restaurants/index.html', {'restaurants': restaurants,'inspections':inspections,'violations':violations }, context_instance = RequestContext( request) )
...
def detail( request,restaurant_id ):
  ...
  return render_to_response(
      'restaurants/detail.html',{'restaurant':restaurant,'inspections':inspections }, context_instance = RequestContext(request) )
@never_cache
def browse(request,letter=None,page=None):
  ...
  restaurants = Restaurant.objects.filter(name__istartswith=letter).order_by('name')
  #pass to paginator
  ...
  return render_to_response( 'restaurants/browse.html', { 'restaurants': p.object_list,
                                                          'letter': letter,
                                                          'alphabet': alphabet,
...
def search( request, page=None ):
  ...
  if request.method == "POST" or request.session.get( "searched", False ):
    ...
    if request.method == "POST":
      ...
      #request.session[ "post" ][ "violation" ] = form.data[ "violation_desc" ]
    ...
    if form.is_valid():
      ...
      if form.cleaned_data[ "name" ] is not None and form.cleaned_data[ "name" ] != "":
        ...
        name_q = None
      #address
      if form.cleaned_data[ "address" ] is not None and form.cleaned_data["address" ] != "":
        address_q = Q( address__icontains = form.cleaned_data["address"] )
      else:
        address_q = None
      #CITY
      if form.cleaned_data[ "city" ] is not None and form.cleaned_data[ "city" ] != "":
  ...
  return render_to_response('restaurants/search.html', content, context_instance = RequestContext( request ))
...
def confirm_merge( request ):
  ...
  return render_to_response('restaurants/confirm_merge.html',{'restaurants':restaurants}, context_instance = RequestContext(request))
