...
columns = { 0: "name", 1: "age", 2: "warrant_type", 3:"bond", 4:"warrant_number", 5:"violation_desc" }
...
for letter in letters:
	doc = urllib.urlopen( 'http://www.greenecountymo.org/sheriff/warrants.php?search=' + str( letter.upper() ) )
	print "Pulling %s from the website" % letter
			...
			if letter_row_count > 0:
				for td in tds:
					...
					if td.find( text=True ) in [ "DFNAME", "AGE","WTYPTX","VILLD1","TKINFO","RELCON", "WRTBND" ]:
						...
						warrant[ columns[ count ] ] = td.find( text=True )
						count += 1
...
for warrant in warrants:
	...
	if "warrant_number" not in warrant.keys():
		...
		try:
			new_warrant = Warrant.objects.get( warrant_number__exact = str( warrant[ "warrant_number" ]), court=court,warrant_type=str(warrant["warrant_type"]),violation_desc=str(warrant["violation_desc"]),bond=str(warrant["bond"]),active=True )
			#print new_warrant
			print "FOUND! Warrant %s already exists" % warrant[ "warrant_number" ]
			update_count += 1
			new_warrant.name = str(warrant[ "name" ])
			if warrant["age"] is not None:
