...
from census.models import CountyData,StateData,TractData,CityData
from lib import bail

rows = csv.reader( open( "data/DC/PlacesDC.csv" ) )
	...
	if count != 1:
		...
		for column in row:
			if col_count in columns:
				if row[ col_count ] == "":
					row[ col_count ] = 0
				#print "row[" + str( col_count )  +"] = " + str( row[ col_count ] )
			col_count += 1
		#print count, row
		...
		try:
			...
			print "There was an error with saving row: %d - %s " %  ( count, cd.city_name )
			error_count +=1
	

print "%d total towns imported" % count
print "%d towns had errors" % error_count
